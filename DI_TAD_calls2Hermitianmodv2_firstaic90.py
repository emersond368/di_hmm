...
from DI8 import DI
from convertbed2v2_62bin import BedConvert
...
def main():
     ...
     parser.add_argument("resolution",type=int,help="resolution basepair")
     ...
     parser.add_argument('label', nargs='?', default="regionX",help ='label output')
     args = parser.parse_args()
 
     data = MatrixMakeBanded(args.count,args.DIrange,args.distance,args.label)

     #print("data.Heatmap: ",data.Heatmap)
     #print(data.Heatmap[2010,205])
     #print(data.Heatmap[2015,195])

     value = DI(data.Heatmap,args.distance,args.DIrange,args.window,False)
     value.DI_metric()
     #value.scramble_DI_metric()

     Data2 = BedConvert(args.bed,value.DIset,args.DIrange,data.start,data.end,args.resolution,args.window,args.distance)
     
     Data3 = TAD_calls(Data2.DI_remove_edge_sparse,args.min_cluster,10)   
     Data3.TAD_calls_Start()  
     np.savetxt('output/'+ args.label + 'aic.csv', Data3.aic, delimiter=',')
     np.savetxt('output/'+ args.label + 'll.csv', Data3.ll, delimiter=',')
     np.savetxt('output/'+ args.label + 'best_p.csv', Data3.best_p, delimiter=',', fmt='%d')
     np.savetxt('output/'+ args.label + 'best_d.csv', Data3.best_d, delimiter=',',fmt='%d')
     np.savetxt('output/'+ args.label + 'best_g1.csv', Data3.best_g1, delimiter=',')
     np.savetxt('output/'+ args.label + 'best_g2.csv', Data3.best_g2, delimiter=',')
     np.savetxt('output/'+ args.label + 'best_g3.csv', Data3.best_g3, delimiter=',')
     np.savetxt('output/'+ args.label + 'comprehensive.csv', Data3.comprehensive, delimiter=',')

     output_name = 'output/'+ args.label + '_postp_list.tsv'
     out_file = open(output_name, 'w')
     output = "chromosome" + '\t' + "start" + '\t' + "end" + '\t' +  "DI" + '\t' + 'best_p' + '\t' + 'best_d' + '\t' + 'best_g1' + '\t' + 'best_g2' + '\t' + 'best_g3'
     print(output, file=out_file)
     for i in range(len(Data2.remove_edge_sparse_comprehensive)):
          chromosome = Data2.remove_edge_sparse_comprehensive[i][0]
          start = Data2.remove_edge_sparse_comprehensive[i][1]
          end = Data2.remove_edge_sparse_comprehensive[i][1] + int(args.resolution)
          directionality_index = Data2.remove_edge_sparse_comprehensive[i][3]
          best_p = Data3.comprehensive[i][0]
          best_d = Data3.comprehensive[i][1]
          best_g1 = Data3.comprehensive[i][2]
          best_g2 = Data3.comprehensive[i][3]
          best_g3 = Data3.comprehensive[i][4]

          output = chromosome + '\t' + str(start) + '\t' + str(end) + '\t' + str(directionality_index) + '\t' + str(best_p) + '\t' + str(best_d) + '\t' + str(best_g1) + '\t' + str(best_g2) + '\t' + str(best_g3)
          print(output, file=out_file)
     out_file.close()
